...
export default function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className="group relative bg-black/40 backdrop-blur-md rounded-2xl p-8 border border-lime-300/20 hover:border-lime-300/40 transition-all duration-300 h-full"
    >
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.95 }}
        className="inline-flex items-center justify-center w-full mt-8 px-6 py-3 text-base font-medium text-white bg-black/40 backdrop-blur-md border border-lime-300/20 rounded-lg hover:border-lime-300/40 transition-all duration-300"
      >
        {service.ctaText}
      </motion.a>
    </motion.div>
